...
class CBFManager:
	...
	def __init__(self, max_cbfs: int, templat: DECBF):
		...
		self._stacked = jax.tree_util.tree_map(lambda leaf: jnp.zeros(
	...
	def add_cbf(self, name:str, cbf:DECBF) -> None:
		...
		self._active_mask = self._active_mask.a[slot].set(True)
	...
	def remove_cbf(self, name:str) -> None:
		...
		if target != last:
			...
			self._name_registry[target] = self._name_regristry[last]
			# target bool mask is true, holding current data

		self._active_mask = self._activ_mask.at[last].set(False)
		del self._name_registry[last]
		self._cbf_count -= 1
		# the mask zeros out old data at compute time

	def get_cbf_linearizations(self, p_agent: jax.Array) -> tuple[jax.Array, jax.Array]:
		"""
		...
		# if mask is 1, then 1*value if mask is False then value becomes Nene
